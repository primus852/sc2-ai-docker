...
class NeuralBot(sc2.BotAI):
    ...
    def on_end(self, game_result):
        ...
        if game_result == Result.Victory:
            np.save("data2/{}.npy".format(str(int(time.time()))), np.array(self.train_data))
    ...
    async def build_zealot(self):
        # if len(self.units(ZEALOT)) < (8 - self.minutes): # how we can phase out zealots over time?
    ...
    async def do_nothing(self):
        ...
        self.do_something_after = self.minutes + wait
    ...
    async def do_something(self):
        ...
        if self.minutes > self.do_something_after:
            if self.use_model:
